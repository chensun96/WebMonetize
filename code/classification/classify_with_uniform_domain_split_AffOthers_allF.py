...
def gird_search_LeaveOneGroupOut(df_labelled, df_labelled_holdout, result_dir, iteration, log_pred_probability):
    ...
    fields_to_remove = ["visit_id", "url_domain", "name", "label", "party", "Unnamed: 0", 'top_level_url', 'Unnamed: 0_x', 'Unnamed: 0_y', 'Unnamed: 0_x_x', 'Unnamed: 0_x_y', 'Unnamed: 0_y_x', 'Unnamed: 0_y_y']
...
def pipeline(df_features, df_labels, df_records, result_dir):
    ...
    df_features.drop(columns=['Unnamed: 0', "Unnamed: 0_x" ,"Unnamed: 0_y", "Unnamed: 0_x_x", "Unnamed: 0_x_y", "Unnamed: 0_y_x", "Unnamed: 0_y_y"], inplace=True, errors="ignore")
...
if __name__ == "__main__":
    ...
    RESULT_DIR = "../../output/results/02_28_uniform_Kfold_CV_AFF_OTHERS_allF/"
    ...
    for crawl_id in os.listdir(others_folder):
        ...
        for filename in os.listdir(each_crawl):
            ...
            elif filename == "url_features.csv":
                file_path = os.path.join(each_crawl, filename)
                df = pd.read_csv(file_path, on_bad_lines='skip')
                df_others_url_features = df_others_url_features.append(df)
            else:
                continue
    ...
    print("len of others url features", len(df_others_url_features))
    ...
    df_affiliate_url_features = pd.DataFrame()
    ...
    for crawl_id in os.listdir(affiliate_folder):
        ...
        for filename in os.listdir(each_crawl):
            ...
            # phase A simple
                ...
                df = pd.read_csv(file_path, on_bad_lines='skip')
                df['visit_id'] = df['visit_id'].astype(str)
                df_affiliate_url_features = df_affiliate_url_features.append(df)
            else:
                continue
    ...
    print("len of affiliate url features ", len(df_affiliate_url_features))
    ...
    df_affiliate_all_features.to_csv("/home/data/chensun/affi_project/purl/df_affiliate_all_features.csv")
    ...
    df_others_all_features.to_csv("/home/data/chensun/affi_project/purl/df_others_all_features.csv")
    print("df_others_all_features: ", len(df_others_all_features))
    print("df_affiliate_all_features: ", len(df_affiliate_all_features))
