...
def read_sql_crawl_data_for_multi_tabs(visit_id, db_file, conn, tab_id, final_page_fld):
    """Read SQL data from crawler for a given visit_ID.
    :param visit_id: visit ID of a crawl URL.
    :return: Parsed information (nodes and edges) in pandas df.
    """
    ...
    # Check graph type
    if final_page_fld in ["facebook.com", "instagram.com", "twitter.com", "youtube.com", "linkedin.com", "pinterest.com"]:
        print("Common social media domain. Type is others")
        graph_type = 'others'
    else:
        graph_type = label_data(df_requests_phase1, df_responses_phase1, df_redirects_phase1)
    ...
    return df_all_graph, graph_type
...
def pipeline(db_file, features_file, ldb_file, subfolder, OUTPUT):
    ...
    for i, row in tqdm(
        ...
        try:
            ...
            aff_graph_folder = os.path.join(OUTPUT, 'affiliate_yt', subfolder)
            non_aff_graph_folder = os.path.join(OUTPUT, 'others_yt', subfolder)

            if not os.path.exists(aff_graph_folder):
                os.makedirs(aff_graph_folder)
            if not os.path.exists(non_aff_graph_folder):
                os.makedirs(non_aff_graph_folder)
            ...
            for i in range(len(unique_tab_ids)):
                try: 
                    ...
                    final_page_fld = str(get_fld(final_page_url)).strip()
                
                    pdf, graph_type = read_sql_crawl_data_for_multi_tabs(visit_id, db_file, conn, unique_tab_ids[i], final_page_fld)

                    if graph_type == 'affiliate':
                        graph_folder = aff_graph_folder
                        print("affiliate link")
                    elif graph_type == 'others':
                        print("other types of link")
                        graph_folder = non_aff_graph_folder
    ...
    for i in ["others", "affiliate"]:
        ...
        graph_type = i 
        graph_folder = os.path.join(OUTPUT, graph_type + '_yt', subfolder)
...
if __name__ == "__main__":
    ...
    BD_PATH = "/home/data/chensun/affi_project/purl_test/OpenWPM_5/datadir_youtube_aff-normal-24000/"
    DB_FILE = "/home/data/chensun/affi_project/purl_test/OpenWPM_5/datadir_youtube_aff-normal-24000/crawl-data.sqlite"
    LDB_FILE = "/home/data/chensun/affi_project/purl_test/OpenWPM_5/datadir_youtube_aff-normal-24000/content.ldb"
